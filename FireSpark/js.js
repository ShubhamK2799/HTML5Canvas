...
function init(i){
    ...
    dr[i]= -drMax*Math.random(); 
    vx[i]=  vMax*(1-2*Math.random());
    vy[i]= -vMax*Math.random();
    switch(Math.floor(5*Math.random())){
     	case 0 :col[i]= "#ef9a0f"; break;
     	case 1 :col[i]= "#efc30f"; break;
     	case 2 :col[i]= "#ff5722"; break;
     	case 3 :col[i]= "#f43636e0"; break;
     	default:col[i]= "#edf436e0"; 
     };	
...
function update(){
  ...
  for(var i=0; i<n;i++){
    ...
    if((x[i] > w+r[i]  || x[i]<-r[i]) || (y[i]> h+r[i]  || y[i]<-r[i]) || r[i]<0.5 || dr[i]>- 0.01)
      init(i); 
